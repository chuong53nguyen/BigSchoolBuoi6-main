...
namespace BigSchool.Controllers
{
    public class HomeController : Controller
    {
        public ActionResult Index()
        {
            ...
            var upcommingCourse = context.Courses.Where(p => p.DateTimea > DateTime.Now).OrderBy(p => p.DateTimea).ToList();
            ...
            foreach (Course i in upcommingCourse)
            {
                ...
                if (userID != null)
                {
                    ...
                    Followinga findFollow = context.Followingas.FirstOrDefault(p =>
                    p.FollowerId == userID && p.FolloweeId == i.LectunerId);
                    if (findFollow == null)
                        i.isShowFollow = true;
                }
            }
